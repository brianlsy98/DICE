...
if __name__ == "__main__":
    ...
    print()
    start = time.time()
    print("Node embeddings PCA (untrained)...")
    pca_node_untrained = PCA(n_components=2, random_state=42)
    node_embeddings_pca_untrained = pca_node_untrained.fit_transform(untrained_node_embeddings)
    end = time.time()
    print(f"done in {end-start:.2f} seconds")

    start = time.time()
    print("Edge embeddings PCA (untrained)...")
    pca_edge_untrained = PCA(n_components=2, random_state=42)
    edge_embeddings_pca_untrained = pca_edge_untrained.fit_transform(untrained_edge_embeddings)
    end = time.time()
    print(f"done in {end-start:.2f} seconds")

    start = time.time()
    print("Graph embeddings PCA (untrained)...")
    pca_graph_untrained = PCA(n_components=2, random_state=42)
    graph_embeddings_pca_untrained = pca_graph_untrained.fit_transform(untrained_graph_embeddings)
    end = time.time()
    print(f"done in {end-start:.2f} seconds")

    start = time.time()
    print("Node embeddings PCA (trained)...")
    pca_node_trained = PCA(n_components=2, random_state=42)
    node_embeddings_pca_trained = pca_node_trained.fit_transform(trained_node_embeddings)
    end = time.time()
    print(f"done in {end-start:.2f} seconds")

    print("Edge embeddings PCA (trained)...")
    start = time.time()
    pca_edge_trained = PCA(n_components=2, random_state=42)
    edge_embeddings_pca_trained = pca_edge_trained.fit_transform(trained_edge_embeddings)
    end = time.time()
    print(f"done in {end-start:.2f} seconds")

    print("Graph embeddings PCA (trained)...")
    start = time.time()
    pca_graph_trained = PCA(n_components=2, random_state=42)
    graph_embeddings_pca_trained = pca_graph_trained.fit_transform(trained_graph_embeddings)
    end = time.time()
    print(f"done in {end-start:.2f} seconds")
    ...
    for i, label in enumerate(unique_node_labels):
        ...
        embeddings = node_embeddings_pca_untrained[indices]
        ax.scatter(embeddings[:, 0], embeddings[:, 1], color=node_cmap(i), label=f'Label {label}', s=15)
    ax.set_title('(Untrained) Node Embeddings PCA')
    ...
    for i, label in enumerate(unique_edge_labels):
        ...
        embeddings = edge_embeddings_pca_untrained[indices]
        ax.scatter(embeddings[:, 0], embeddings[:, 1], color=edge_cmap(i), label=f'Label {label}', s=15)
    ax.set_title('(Untrained) Edge Embeddings PCA')
    ax.set_xlabel('Principal Component 1')
    ax.set_ylabel('Principal Component 2')
    # ax.legend(fontsize='small')
    ...
    for i, label in enumerate(unique_graph_labels):
        ...
        embeddings = graph_embeddings_pca_untrained[indices]
        ax.scatter(embeddings[:, 0], embeddings[:, 1], color=graph_cmap(i), label=f'Label {label}', s=15)
    ax.set_title('(Untrained) Graph Embeddings PCA')
    ax.set_xlabel('Principal Component 1')
    ax.set_ylabel('Principal Component 2')
    # ax.legend(fontsize='small')
    ...
    for i, label in enumerate(unique_node_labels):
        indices = np.where(node_labels_all == label)
        embeddings = node_embeddings_pca_trained[indices]
        ax.scatter(embeddings[:, 0], embeddings[:, 1], color=node_cmap(i), label=f'Label {label}', s=15)
    ax.set_title('(Trained) Node Embeddings PCA')
    ax.set_xlabel('Principal Component 1')
    ax.set_ylabel('Principal Component 2')
    # ax.legend(fontsize='small')
    ...
    for i, label in enumerate(unique_edge_labels):
        indices = np.where(edge_labels_all == label)
        embeddings = edge_embeddings_pca_trained[indices]
        ax.scatter(embeddings[:, 0], embeddings[:, 1], color=edge_cmap(i), label=f'Label {label}', s=15)
    ax.set_title('(Trained) Edge Embeddings PCA')
    ax.set_xlabel('Principal Component 1')
    ax.set_ylabel('Principal Component 2')
    # ax.legend(fontsize='small')
    ...
    for i, label in enumerate(unique_graph_labels):
        indices = np.where(graph_labels_all == label)
        embeddings = graph_embeddings_pca_trained[indices]
        ax.scatter(embeddings[:, 0], embeddings[:, 1], color=graph_cmap(i), label=f'Label {label}', s=15)
    ax.set_title('(Trained) Graph Embeddings PCA')
    ax.set_xlabel('Principal Component 1')
    ax.set_ylabel('Principal Component 2')
    # ax.legend(fontsize='small')
